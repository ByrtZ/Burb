                ...
                when(GameManager.getGameState()) {
                    ...
                    GameState.GAME_END -> {
                        ...
                        if(CapturePoints.isSuburbinating()) {
                            scoreBossBar.name(Formatting.allTags.deserialize("<translate:space.0>").append(TextAlignment.centreBossBarText("<plantscolour>PLANTS<white>: ${Scores.getDisplayScore(Teams.PLANTS)} <gray>| <zombiescolour>ZOMBIES<white>: ${Scores.getDisplayScore(Teams.ZOMBIES)}")).append(Formatting.allTags.deserialize("<translate:space.100>")).append(TextAlignment.centreBossBarText("<u>${CapturePoints.getSuburbinatingTeam().teamColourTag}SUBURBINATION")))
                        } else {
                            ...
                            scoreBossBar.name(Formatting.allTags.deserialize("<translate:space.0>").append(TextAlignment.centreBossBarText("<plantscolour>PLANTS<white>: ${Scores.getDisplayScore(Teams.PLANTS)} <gray>| <zombiescolour>ZOMBIES<white>: ${Scores.getDisplayScore(Teams.ZOMBIES)}")).append(Formatting.allTags.deserialize("<translate:space.100>")).append(TextAlignment.centreBossBarText("${if(pointA.second == Teams.PLANTS) "<plantscolour>" else if(pointA.second == Teams.ZOMBIES) "<zombiescolour>" else "<speccolour>"}${pointA.first}<white>% <gray>| ${if(pointB.second == Teams.PLANTS) "<plantscolour>" else if(pointB.second == Teams.ZOMBIES) "<zombiescolour>" else "<speccolour>"}${pointB.first}<white>% <gray>| ${if(pointC.second == Teams.PLANTS) "<plantscolour>" else if(pointC.second == Teams.ZOMBIES) "<zombiescolour>" else "<speccolour>"}${pointC.first}<white>%")))
                        }
                    }
                }
                ticks++
