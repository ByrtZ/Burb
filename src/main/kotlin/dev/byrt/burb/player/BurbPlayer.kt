...
class BurbPlayer(val uuid: UUID, val playerName: String, var playerType: PlayerType, var playerTeam: Teams, var playerCharacter: BurbCharacter) {
    ...
    fun setCharacter(newCharacter: BurbCharacter) {
        ...
        if(this.playerCharacter != BurbCharacter.NULL && (this.getBukkitPlayer().vehicle == null || this.getBukkitPlayer().vehicle?.scoreboardTags?.contains("${this.getBukkitPlayer().uniqueId}-death-vehicle") == false)) {
